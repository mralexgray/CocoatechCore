...
- (BOOL)splitView:(NSSplitView *)splitView shouldCollapseSubview:(NSView *)subview forDoubleClickOnDividerAtIndex:(NSInteger)dividerIndex;
{
	...
	int collapseIndex = [self collapseViewIndex];
	if (collapseIndex != -1)
	{
		NSArray* views = [splitView subviews];
		if ([views count] == 2)
						  ...
						  oldSize:(NSSize)oldSize;
{
	...
	if (![splitView isVertical])
	{
		...
		secondRect.size.height = (int) ceil(ratio * [splitView bounds].size.height);
	...
	else
	{
		...
		secondRect.size.width = (int) ceil(ratio * [splitView bounds].size.width);
