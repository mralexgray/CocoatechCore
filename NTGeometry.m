...
NSRect NTClosestRectToRect(NSRect sourceRect, NSArray *candidateRects)
{
    ...
    if (rectIndex > 0)
	{
        ...
        float shortestDistance = NTSquaredDistanceToFitRectInRect(sourceRect, rect);
        closestRect = rect;
		
        while (rectIndex-- > 0) 
		{
            NSRect iteratedRect = [(NSValue *)[candidateRects objectAtIndex:rectIndex] rectValue];
            float distance = NTSquaredDistanceToFitRectInRect(sourceRect, iteratedRect);
...
float NTSquaredDistanceToFitRectInRect(NSRect sourceRect, NSRect destinationRect)
{
