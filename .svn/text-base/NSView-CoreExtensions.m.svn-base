...
{
	NSWindow* window = [[NSView focusView] window];
	return [window isMetallic];
}
...
- (BOOL)isVisible;
{
	...
	// not sure if this is needed, but makes sense.  View heirarchy no in any window, so not visible
	if (![self window])
		return NO;
