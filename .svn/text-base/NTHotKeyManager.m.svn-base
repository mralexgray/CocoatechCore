    ...
    if (eventKind == kEventHotKeyPressed)
    {
        ...
        if (err == noErr)
        {
			if (hotKeyID.signature == kHotKeySignature)			
				[[NSNotificationCenter defaultCenter] postNotificationName:kNTHotKeyManagerNotification object:[NTHotKeyManager sharedInstance] userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:hotKeyID.id], @"identifier", nil]];
        }
    }
