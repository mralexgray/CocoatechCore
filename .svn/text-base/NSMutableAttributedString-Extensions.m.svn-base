...
+ (NSMutableAttributedString*)stringWithWordURL:(NSURL*)theURL;
{
	...
	/* old code that crashes
	 NSData *data = [NSData dataWithContentsOfFile:[desc path]];
	 if (data)
	 {
	 NSAttributedString* attributedString = [[[NSAttributedString alloc] initWithDocFormat:data documentAttributes:nil] autorelease];
	 
	 if (attributedString)
	 {
	 [self normalizeTextView];
	 [[mv_textPreviewView textStorage] setAttributedString:attributedString];
	 }
	 else
	 [mv_textPreviewView setString:@""];
	 }
	 */
