...
// not certain that this does anything, but hopefully it helps
...
- (void)drawCenteredInRectHQ:(NSRect)rect operation:(NSCompositingOperation)op fraction:(float)delta;
{
	...
	drawRect = [NTGeometry rect:imageRect centeredIn:rect scaleToFitContainer:YES];
	drawRect = [NTGeometry integerRect:drawRect];
		
	[self drawInRectHQ:drawRect fromRect:NSZeroRect operation:op fraction:delta];
}
...
- (NSImage*)copyImageWithShadow:(NSShadow*)shadow;
{
	...
	[self compositeToPointHQ:NSMakePoint(0,0) operation:NSCompositeSourceOver fraction:1.0];
...
- (NSImage*)coloredImage:(NSColor*)color;
{
	...
	[self compositeToPointHQ:NSMakePoint(0,0) operation:NSCompositeDestinationIn fraction:1.0];
...
- (NSImage*)imageWithAlpha:(float)theAlpha;
{
	...
	[self compositeToPointHQ:NSMakePoint(0,0) operation:NSCompositeSourceOver fraction:theAlpha];
...
- (NSImage*)imageWithMaxSize:(int)maxSize;
{
	...
	[self drawInRectHQ:newRect fromRect:imageRect operation:NSCompositeSourceOver fraction:1.0];
...
- (NSImage*)imageWithSize:(NSSize)theSize scaleLarger:(BOOL)scaleLarger;
{
	...
	if (!NSEqualSizes(srcSize, theSize))
	{
		...
		[self drawInRectHQ:imageRect fromRect:srcRect operation:NSCompositeSourceOver fraction:1.0];
...
- (NSImage*)imageWithTopMargin:(int)topMargin;
{
	...
	[self compositeToPointHQ:destRect.origin operation:NSCompositeSourceOver fraction:1];
...
- (NSBitmapImageRep*)bitmapImageRepForSize:(int)size;
{
    ...
    NTImageMaker *imageMaker = [NTImageMaker maker:NSMakeSize(size, size)];
...
- (NSImage*)flip;
{
	...
	if ((size.height > 0) && (size.width > 0))
	{
		...
		[self drawInRectHQ:drawRect fromRect:NSMakeRect(0, 0, size.width, size.height) operation:NSCompositeCopy fraction:1.0];
		
		// composite calls ignore the graphics state
		// [self compositeToPointHQ:drawPoint operation:NSCompositeCopy fraction:1.0];
...
{
	return [self CGImageFromData:[image TIFFRepresentation]];
}

+ (NSImage*)imageFromCGImageRef:(CGImageRef)image
{
    NSRect imageRect = NSMakeRect(0.0, 0.0, 0.0, 0.0);
    CGContextRef currentContext = nil;
    ...
    currentContext = (CGContextRef)[[NSGraphicsContext currentContext]
								  graphicsPort];
	
    // draw images at highest possible quality
    CGInterpolationQuality imageInterpolation = CGContextGetInterpolationQuality(currentContext);
	CGContextSetInterpolationQuality(currentContext, kCGInterpolationHigh);
    
    CGContextDrawImage(currentContext, *(CGRect*)&imageRect, image);

	CGContextSetInterpolationQuality(currentContext, imageInterpolation);
...
{	
	...
	[self drawCenteredInRectHQ:contentRect operation:NSCompositeSourceOver fraction:1.0];
	[controlImage compositeToPointHQ:arrowRect.origin operation:NSCompositeSourceOver fraction:1.0];
...
+ (NSImage*)image:(NSRect)bounds interior:(NSImage*)interior backColor:(NSColor*)backColor;
{
	...
	[interior compositeToPointHQ:interiorBounds.origin operation:NSCompositeSourceOver fraction:1.0];
...
{
    NSBezierPath* copy, *linePath = [NSBezierPath newPath];
...
{
    NSBezierPath *linePath = [NSBezierPath newPath];
